...
(margin?num(this,"margin"+tl)+num(this,"margin"+br):0);};});})();jQuery.fn.delay=function(time,func){this.each(function(){setTimeout(func,time);});return this;};(function($){$.fn.jCarouselLite=function(o){o=$.extend({btnPrev:null,btnNext:null,btnGo:null,mouseWheel:false,auto:null,speed:200,easing:null,vertical:false,circular:true,visible:3,start:0,scroll:1,beforeStart:null,afterEnd:null},o||{});return this.each(function(){var running=false,animCss=o.vertical?"top":"left",sizeCss=o.vertical?"height":"width";var div=$(this),ul=$("ul",div),tLi=$("li",ul),tl=tLi.size(),v=o.visible;if(o.circular){ul.prepend(tLi.slice(tl-v-1+1).clone()).append(tLi.slice(0,v).clone());o.start+=v;}
...
jQuery.fn.unwrap=function(){return this.each(function(){$(this.childNodes).insertBefore(this);});};$(document).ready(function(){$("#new_comment").submit(function(){$.post(this.action,$(this).serialize(),null,"script");return false;})})
var forms=new Array(".new_project input",".new_project textarea",".new_page input",".new_page textarea",".new_user input");$(document).ready(function(){$(forms.join(", ")).keyup(function(){current_element=$(this);var form=$(this).parents().filter('form');$(this).delay(800,function(){var location=form.attr("action").toString()
location=location+"/validate"
$.get(location,form.serialize(),display_validation_errors,"json")
return false})})})
function display_validation_errors(data){$("div.fieldWithErrors").unwrap().remove();$("span.errorMessage").remove();jQuery.each(data.errors,function(index,value){var name=value[0];var message=value[1];var field=$("#"+data.model+"_"+name);var label=$("label[for="+data.model+"_"+name+"]");if(!field.parent().hasClass("fieldWithErrors")){label.after("<span class='errorMessage'> - "+name+" "+message+" </span>");if(name!="subdomain"){field.wrap("<div class='fieldWithErrors'></div>");}}});window.current_element.focus();}
$(document).ready(function(){$('.question_link').click(function(){$.get($(this).attr("href"),null,null,"script");return false;})})
$(document).ready(function(){$(".feature_carousel").jCarouselLite({btnNext:".next",btnPrev:".prev",visible:1,circular:false});});$(document).ready(function(){$(".show_video_1").click(function(e){$('#video_1_container').css("display","inline");$('.show_video_1').css("display","none");e.preventDefault();});});$(document).ready(function(){$(".show_video_2").click(function(e){$('#video_2_container').css("display","inline");$('.show_video_2').css("display","none");e.preventDefault();});});$(document).ready(function(){$(".show_video_3").click(function(e){$('#video_3_container').css("display","inline");$('.show_video_3').css("display","none");e.preventDefault();});});$(document).ready(function(){$('.next').click(function(){stop_videos();})})
$(document).ready(function(){$('.prev').click(function(){stop_videos();})})
function stop_videos(){var player_ids=new Array('video_1','video_2','video_3');for(var x in player_ids){try{document.getElementById(player_ids[x]).sendEvent("STOP");$('#'+player_ids[x]+'_container').css("display","none");$('.show_'+player_ids[x]).css("display","inline");}catch(err){null;};};}